      ...
      {open && (
        <div className="touristsPicker">
          <div className="children">
            <div className="touristsPicker__title">Детей</div>
            <ul>
              {[...Array(6)].map((obj, i) => (
                <li
                  key={i}
                  onClick={() => onClickChild(i)}
                  className={i === childrenCount ? 'activeTouristsPicker' : ''}>
                  {i}
                </li>
              ))}
            </ul>
          ...
          <div className="adults">
            <div className="touristsPicker__title">Взрослых</div>
            <ul>
              {[...Array(6)].map((obj, i) => (
                <li
                  key={i}
                  onClick={() => onClickAdult(i)}
                  className={i === adultsCount ? 'activeTouristsPicker' : ''}>
                  {i}
                </li>
              ))}
            </ul>
